  ...
  (cond
   ...
   [else
    (if (> (elem-priority (heap-min h1)) (elem-priority (heap-min h2)))
        (let ((nh2 (heap-merge (node-left h2) (node-right h2))))
              (list 'hnode (heap-min h2) (+ 1 (rank nh2)) h1 nh2))
        (list 'hnode (heap-min h1) (+ 1 (rank h2)) (heap-merge (node-left h1) (node-right h1)) h2))]))
...
(heapsort (list 52 23 12 1 324 23 235)) ;; It returns '(1 12 23 23 52 235 324) and it is sorted