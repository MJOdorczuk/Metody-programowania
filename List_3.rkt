...
(define (solve-ax+by=1 a b)
  (if (< a b) (reverse-pair (solve-ax+by=1 b a))
      (let ([r (modulo a b)])
        (let ([q (/ (- a r) b)])
          (if (= r 1) (cons 1 (- q))
              (let ([in (solve-ax+by=1 b r)])
                (cons (cdr in) (+ (car in) (* (cdr in) (- q))))))))))
...
(define (RSA-unconvert-list intlist key)
  (let ([m (key-modulus key)])
    (define (convert l sum)
      (if (null? l) '()
          (let ([x (modulo (+ (RSA-transform (car l) key) sum) m)])
...
(define (make-signed-message message)
  (cons (compress message) message))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (let ([encrypted (RSA-encrypt message recipient-public-key)])
    (make-signed-message encrypted)))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (let ([sign (compress (message cyphertext))])
    (if (= sign (signature cyphertext))
...
(encrypt-and-sign "Vollo illo!" (key-pair-public test-key-pair1) (key-pair-private test-key-pair1))
(authenticate-and-decrypt (encrypt-and-sign "Vollo illo!" (key-pair-private test-key-pair1) (key-pair-public test-key-pair1)) (key-pair-public test-key-pair1) (key-pair-private test-key-pair1))