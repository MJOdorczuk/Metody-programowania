...
(define (intersect s1 s2)
  (define (in-inter s1 s2 acc)
    (if (contains? s1 (car s2))
        (in-inter s1 (cdr s2) (cons (car s2) acc))
        (in-inter s1 (cdr s2) acc)
        )
    )
  (in-inter s1 s2 null)
  )

(define (sub-list s1 s2)
  (if (contains? s2 (car s1))
      (sub-list (cdr s1) s2)
      (cons (car s1) (sub-list (cdr s1) s2))
      )
  )

(define (merge-inter c1 c2)
  (let ((p1 (res-clause-pos c1))
        (p2 (res-clause-pos c2))
        (n1 (res-clause-neg c1))
        (n2 (res-clause-neg c2)))
    (let ((i1 (intersect p1 n2))
          (i2 (intersect p2 n1)))
      (let ((p (remove-duplicates-vars
                (merge-vars p1 p2)))
            (n (remove-duplicates-vars
                (merge-vars n1 n2)))
            (i (remove-duplicates-vars
                (merge-vars i1 i2)))
            )
        (let ((pc (sub-list p i))
              (nc (sub-list n i)))
...
(define (clause-trivial? c)
  (if (eq? (intersect (res-clause-pos c) (res-clause-neg c)) null)
      false
      true))
